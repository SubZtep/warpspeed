...
const changeData = comp => prop => val => void boxEl.setAttribute(comp, {
  [prop]: val
});

const testWorker = uw => {
  if (uw && !boxEl.is("worker")) {
    boxEl.setAttribute("warpspeed", {
      useWorker: false
    });
    showError();
  }
...
const attrify = obj => Object.entries(obj).map(([key, value]) => `${key}: ${value}`).join("; ");
...
setAttr("w");
setAttr("h");
sceneEl.addEventListener("animationtimelinecomplete", () => void setTimeout(() => {
  boxEl.setAttribute("warpspeed", {
    width: 512,
    useWorker: true
  });
