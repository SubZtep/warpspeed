const I = { r: 255, g: 255, b: 255 },
  s = s =>
    (I => "#" === I.charAt(0) && 7 === I.length)(s)
      ? (I => ({ r: parseInt(I.substr(1, 2), 16), g: parseInt(I.substr(3, 2), 16), b: parseInt(I.substr(5, 2), 16) }))(
          s
        )
      : (I => I.startsWith("rgba"))(s)
      ? (I => {
          const [, s, F, U, g] = /^\s*rgba\s*\(\s*(\d+)\s*,\s*(\d+)\s*,\s*(\d+)\s*,\s*(\d+)\s*\)\s*$/.exec(I)
          return { r: s, g: F, b: U }
        })(s)
      : I,
  F = () => 1e3 * (Math.random() - 0.5)
class U {
  constructor(I, s, F) {
    ;(this.x = I), (this.y = s), (this.z = F), (this.size = 0.5 + Math.random())
  }
}
class g {
  config = {}
      ...
      if (this.config[C] !== c) {
        switch (C) {
          ...
          case "starScale":
            ...
            break
          case "density":
            const I = Math.ceil(1e3 * c),
              C = I - this.stars.length
            if (C > 0)
              for (let I = 0; I < C; I++) {
                const I = new U(F(), F(), 1e3 * Math.random())
                this.stars.push(I)
              }
            else this.stars.length = I
            break
          case "starColor":
            const B = s(c)
            ;(this.starR = B.r), (this.starG = B.g), (this.starB = B.b)
        }
        this.config[C] = c
      }
    g &&
  ...
  draw(I) {
    this.move(I)
    const s = this.config.width,
      F = this.config.height
    this.size = Math.max(s, F) / (10 / this.config.starScale)
    const U = this.ctx
    ;(U.fillStyle = this.config.backgroundColor), U.fillRect(0, 0, s, F)
    for (const I of this.stars) {
      const g = I.x / I.z,
        C = I.y / I.z
      if (!this.config.warpEffect && (g < -0.5 || g > 0.5 || C < -0.5 || C > 0.5)) continue
      const c = (I.size * this.size) / I.z
      if (!(c < 0.3)) {
        if (this.config.depthAlpha) {
          const s = (1e3 - I.z) / 1e3
          U.fillStyle = `rgba(${this.starR}, ${this.starG}, ${this.starB}, ${s.toString()})`
        } else U.fillStyle = this.config.starColor
        if (this.config.warpEffect) {
          U.beginPath()
          const B = I.x / (I.z + this.config.warpEffectLength * this.config.speed),
            Q = I.y / (I.z + this.config.warpEffectLength * this.config.speed)
          if (B < -0.5 || B > 0.5 || Q < -0.5 || Q > 0.5) continue
          U.moveTo(s * (g + 0.5) - c / 2, F * (C + 0.5) - c / 2),
            U.lineTo(s * (B + 0.5) - c / 2, F * (Q + 0.5) - c / 2),
            (U.lineWidth = Math.min(c, this.maxLineWidth)),
            (U.lineCap = this.config.useCircles ? "round" : "butt"),
            (U.strokeStyle = U.fillStyle),
            U.stroke()
        } else
          this.config.useCircles
            ? (U.beginPath(), U.arc(s * (g + 0.5) - c / 2, F * (C + 0.5) - c / 2, c / 2, 0, 2 * Math.PI), U.fill())
            : U.fillRect(s * (g + 0.5) - c / 2, F * (C + 0.5) - c / 2, c, c)
      }
    }
  }
  move(I) {
    const s = this.config.speed * (I / 10)
    for (const I of this.stars)
      for (I.z -= s; I.z < 1; ) (I.z += 1e3), (I.x = (Math.random() - 0.5) * I.z), (I.y = (Math.random() - 0.5) * I.z)
  }
}
function C(I, s, F) {
  var U = void 0 === s ? null : s,
    g = (function (I, s) {
      var F = atob(I)
      if (s) {
        for (var U = new Uint8Array(F.length), g = 0, C = F.length; g < C; ++g) U[g] = F.charCodeAt(g)
        return String.fromCharCode.apply(null, new Uint16Array(U.buffer))
      }
      return F
    })(I, void 0 !== F && F),
    C = g.indexOf("\n", 10) + 1,
    c = g.substring(C) + (U ? "//# sourceMappingURL=" + U : ""),
...
var c,
  ...
  L =
    ...
    (Q = !1),
    function (I) {
      return (i = i || C(c, B, Q)), new Worker(i, I)
    })
AFRAME.registerComponent("warpspeed", {
  schema: {
    ...
    density: { type: "number", default: 25.7 },
    useCircles: { type: "boolean", default: !0 },
    depthAlpha: { type: "boolean", default: !0 },
    warpEffect: { type: "boolean", default: !0 },
    warpEffectLength: { type: "number", default: 5 },
    starScale: { type: "number", default: 3 },
    backgroundColor: { type: "color", default: "#100a1a" },
    starColor: { type: "color", default: "#ffffff" },
    useWorker: { type: "boolean", default: !1 },
  },
  update(I) {
    ;(I.useWorker !== this.data.useWorker || I.width !== this.data.width || I.height !== this.data.height) &&
      (void 0 !== I.useWorker && this.destroyCanvasMap(), this.createCanvasMap(), this.initWarpspeed()),
      this.workerInUse()
        ? this.worker.postMessage({ config: this.data, pause: !this.isPlaying })
        : this.warpspeed.update(this.data)
  },
  initWarpspeed() {
    if ((this.el.removeState("worker"), this.data.useWorker))
      try {
        const I = this.canvas.transferControlToOffscreen()
        return (this.worker = new L()), this.worker.postMessage({ canvas: I }, [I]), void this.el.addState("worker")
      } catch (I) {
        console.error(I)
      }
    this.warpspeed = new g(this.canvas)
  },
  createCanvasMap() {
  ...
  destroyCanvasMap() {
    this.workerInUse() ? (this.worker.terminate(), delete this.worker) : delete this.warpspeed,
      this.canvasMap.dispose(),
      delete this.canvas
  },
  play() {
    this.workerInUse() && this.worker.postMessage({ pause: !1 })
  },
  pause() {
    this.workerInUse() && this.worker.postMessage({ pause: !0 })
  },
  workerInUse() {
    return this.el.is("worker")
  },
  tick(I, s) {
    this.workerInUse() || this.warpspeed.draw(s), (this.canvasMap.needsUpdate = !0)
  },
})
